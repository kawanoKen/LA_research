    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            for self.batch in self.dataset:
                ...
                # def save_img(im, figname):
                #     if len(im.squeeze().shape) == 3:
                #         plt.imshow(im.cpu().clone().squeeze().permute(1, 2, 0).numpy())
                #     elif len(im.squeeze().shape) == 2:
                #         plt.imshow(im.cpu().clone().squeeze().numpy())
                #     plt.savefig(f'{figname}.png')
                # save_img(im, 'tmp')
                # def get_first_layer_input(module, input, output, layer_id):
                #     # input はタプル形式になっているため、最初の要素を取り出す
                #     first_layer_input = input[0]
                #     if layer_id == 0:
                #         print("最初のレイヤへの入力の形状:", first_layer_input.shape)
                #         save_img(first_layer_input, f'{layer_id}-in')
                #     for i in range(output[0].shape[0]):
                #         plt.subplot(1, output[0].shape[0], i+1)
                #         plt.imshow(output[0][i].to('cpu').detach().numpy().copy(), cmap='viridis')  # または 'hot', 'jet', 'inferno' など
                #         plt.axis('off')
                #         plt.title(f'Channel {i}')
                #         plt.savefig(f'{i}-layer.png')

                # for layer_id, _layer in enumerate(self.model.model.model):
                #     get_layer = partial(get_first_layer_input, layer_id=layer_id)
                #     hook_handle = _layer.register_forward_hook(get_layer)

                # # forward hook を登録（hook_handle により後で解除可能）
