...
edit_label_dir = "/work/kawano/LA/datasets/african-wildlife_noise_generate/train/labels"
...
def delete_files_not_starting_with_1(label_dir,):
...
def edit_yolo_labels(label_dir, edit_label_dir):
    for file_name in os.listdir(label_dir):
        ...
        if random.random()<0.1:
            ...
            x, X = float(min(position[0], position[1])), float(max(position[0], position[1]))
            y, Y = float(min(position[2], position[3])), float(max(position[2], position[3]))
            new_parts_xywh = [1, round((x + X)/2, 6) , round((y + Y)/2, 6), round(X - x, 6), round(Y - y, 6), 0]
            parts.append(list(map(str, new_parts_xywh)))
...
delete_files_not_starting_with_1(edit_label_dir)
